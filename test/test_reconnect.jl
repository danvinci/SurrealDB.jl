...
@testset "events(client) returns the right channel for each transport" begin
    ...
    client_ws = SurrealDB.SurrealClient(conn_ws, nothing, nothing, nothing, Dict{String, Any}())
    ...
    client_http = SurrealDB.SurrealClient(conn_http, nothing, nothing, nothing, Dict{String, Any}())
